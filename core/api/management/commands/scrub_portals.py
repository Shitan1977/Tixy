...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if source == "ticketmaster":
            from api.scrapers.ticketmaster import fetch_events

            data = fetch_events(page_size=limit, country_code="IT")
            ...
            for e in events:
                ...
                slug = f"{evento_norm}-{(tm_id or '')[:8]}".strip("-")

                if dry_run:
                    self.stdout.write(f"[DRY] EVENTO: {name} | {starts_at} | {venue_name} ({city},{country}) | tm_id={tm_id}")
                    continue
